...
AND - 複数条件指定時に使用。 条件1 且つ、条件2等。  
(例)カラム名1が指定条件1、且つ、カラム名2が指定条件2のデータを抽出
select * from テーブル名 where カラム名1 = 指定条件1 and カラム名2 = 指定条件2;  
  
OR - 複数条件指定時に使用。 条件1 もしくは、条件2等。
(例)カラム名1が指定条件1、もしくは、カラム名2が指定条件2のデータを抽出
select * from テーブル名 where カラム名1 = 指定条件1 or カラム名2 = 指定条件2;  

LIKE - 値を前方一致、後方一致もしくは、前後方一致で絞り込む。  
(例)カラム名1が「aaa」で始まるデータを抽出
